...
More than likely however, you will need to _simulate your data_ to answer your question. Luckily, you have those two hats, which will open up worlds of possibilities, and be the key to solving this life-long question.


### What is simulation?
Simulating your data means you have the power to determine what the data are and how they get generated. This is called the _Data Generating Process_, and will be discussed later in this app. For now, understand that simulation starts all the way at the beginning: who is in your sample, and what are their pre-treatment test scores? 

Let's put the _omniscient_ hat on, since it feels good to know everything.
With this powerful hat, you know that you have a sample of 100 fake students (that you will generate), who must be randomly assigned (by some function you will specify) into treatment and control groups. "Treatment" in this case is the afterschool program, and thus those in the treatment group will go through the afterschool program, and those in the control group will receive normal tutoring. You will also be generating all of these students' "pre-treatment" test scores, as well as their "post-treatment" test scores, otherwise known as the _outcome_. These are fairly self-explanatory: **pre-treatment** test scores are the scores of the 100 sample students prior to any of them going through the afterschool program, and **post-treatment** test scores are the scores of the 100 sample students after the treatment period. The great thing about being omniscient is that you will also be able to see what the treatment group students' scores are if they don't receive the treatment. 
...
Moreover, since the only limitation of simulation is computational, it can generate as many new results as you need, and represents the characterstics of the model you defined.

For example, when you are randomly assigning students to receive the treatment, your model would pick 50 students randomly, thus each simulation will pick 50 students randomly.
...
**Vera NOTE: responding to Heidi's comment: as discussed in our story board, we shall consider introduce the concept of simulation here, since we technically already start to simulate in the next session. And we probably need to explain some terms before we use them in the following session(pre-treatment,outcome,etc).**
