...
Suppose you're playing a game of drawing balls with numbers labeled from 1 to infinite from a urn. When a ball is drawn at random, the “random variable” is what is written on this ball. The probability of getting each possible number from the random variable is its distribution. Distributions are about the population. In this game, the probability distribution corresponds to the urn with a potentially infinite number of balls inside. 
...
> Example: binary data (i.e., 0 or 1), count data, Likert scales (on a scale of 1 to 5).
