...
You are an educational researcher tasked by the principal of a local high school to determine whether or not a new afterschool pilot program is effective in improving students' Global History regenets scores. The high school has secured the participation of 100 students (out of 2,000), and you now turn to face your daunting task. 
...
Suppose you're drawing students with pre-treatment scores possibly from 0 to 100 from high schools in New York state. When a student is drawn at random, the “random variable” is what is the pre-treatment of the student. The probability of getting each possible score from the random variable is its distribution. Distributions are about the population. In this example, the probability distribution corresponds to New York state with a potentially infinite number of students inside. 

You can think about a regular (deterministic) function like a box. For example, for $f(x) = x^2$, *every time* you give the function the value 2, and it will give you back 4. However, Random variables are random functions. Each time you apply the function it gives you a different number.


## draw one student & draw 100 students
...
observeEvent(input$draw_hundred_student, {
  ...
  output$hundred_students_scores <- renderPlot(hist(df$score))
})
```



There are generally two types of random variables, either discrete or continuous.

1. Discrete random variables can only take on a countable number of values (possibly infinite, but oftentimes finite).

> Treatments randomly assigned to students (i.e., generate a binary variable of 0 or 1 to students)

```{r, echo=FALSE}
fluidPage(
  actionButton("one_student_treatment", "Draw a student"),
...
```{r, echo=FALSE}
fluidPage(
  actionButton("hundred_student_treatment", "Draw 100 students"),
...
2. Continuous random variables can take on any real number, an uncountable amount of possibilities (i.e., to any amount of decimal places).
> Example: most measurements with reasonable resolution (say, e.g., weight or GPA);

Areas of application of probability distributions include:
- Distributions of data (for example, heights of men, incomes of women, political party preference).
- Distributions of error terms in regression models, $\epsilon$ in the $y = a + bx + \epsilon$.
> Probabilistic distributions are used in regression modeling to help us characterize the variation that remains after predicting the average, a + bx.
...
Third, regression models are not deterministic; they produce probabilistic predictions. Simulation is the most convenient and general way to represent uncertainties in forecasts. Throughout
